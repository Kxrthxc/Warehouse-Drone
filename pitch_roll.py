...
I_LIMIT_X   = 200.0
X_LPF_ALPHA = 0.35
X_REVERSE   = True     # flip if roll moves opposite
...
I_LIMIT_Y    = 200.0
Y_LPF_ALPHA  = 0.35
Y_REVERSE    = False    # flip if pitch moves opposite
...
print("▶ Running: X/Y/Z + Angle print  +  X-PID->ROLL (CH1) + Y-PID->PITCH (CH2)")
...
while True:
    ...
    pitch_rc    = RC_MID
    yaw_rc      = RC_MID
    throttle_rc = RC_MIN  # safety for testing
    ...
    if marker_seen:
        ...
        if y_filt is None:
            ...
            y_filt = (Y_LPF_ALPHA * y_mm) + ((1.0 - Y_LPF_ALPHA) * y_filt)
        ...
        if (now - last_send) >= SEND_DT:
            ...
            # keep yaw neutral + throttle low (safe testing)
            yaw_rc = RC_MID
            throttle_rc = RC_MIN
        ...
        # overlay
        cv2.putText(frame,
                    f"X:{x_filt:+.1f}mm R:{roll_rc}  Y:{y_filt:+.1f}mm P:{pitch_rc}  Z:{z_mm:+.1f}  ANG:{angle_deg:+.1f}",
                    (10, 30),
    ...
    cv2.imshow("Aruco X/Y PID -> Roll/Pitch", frame)
